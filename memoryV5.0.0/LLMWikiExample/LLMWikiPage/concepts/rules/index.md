...
related:
  ...
  - "[[runtime-texture-compression.md]]"
...
**[[concepts/rules/udon-vm-architecture|udon-vm-architecture]]** — VM 内部机制
...
### 🖼️ Runtime Texture Compression

**[runtime-texture-compression.md](runtime-texture-compression.md)** — VRChat 2026.2.2 客户端 GPU 运行时纹理压缩

涵盖：
- ASTC/BC7 压缩路径
- 异步执行与约 75% RAM/VRAM 节省目标
- 客户端资源管理与 Udon API 边界

---

### 🤝 多 VM 协作约束 (RULE-MV-*)

**[[concepts/rules/multi-vm-rules|multi-vm-rules]]** — 9 条规则
...
**[[concepts/rules/udonsharp-deep-pitfalls|udonsharp-deep-pitfalls]]** — 19 条规则
...
**[[concepts/rules/vrchat-api-exposure|vrchat-api-exposure]]** — API 暴露规则
...
> 完整严重度模型见 `（见源库 reviews/ 目录）severity-model.md`
...
| 关系 | 目录 |
|------|------|
| **设计模式（合规实现）** | [[concepts/patterns/index]] |
| **审查清单（如何检查）** | （见源库 reviews/ 目录） |
| **失败案例（违反后果）** | （来源：常见失败案例分析，本地整理） |
| **API 速查（暴露状态）** | [[entities/api/index]] |
