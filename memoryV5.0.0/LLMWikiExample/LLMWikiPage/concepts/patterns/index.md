...
## Obsidian MOC — Patterns 可点击入口

| 分类 | 入口 |
|---|---|
| 核心同步模式 | [[manual-sync-state.md]] · [[owner-authoritative-interaction.md]] · [[late-joiner-state-restore.md]] · [[bit-packed-flags.md]] · [[event-driven-state-machine.md]] · [[advanced-sync-patterns.md]] · [[unorthodox-patterns.md]] |
| LuraSwitch2 模式 | [[master-follower-syncer.md]] · [[exclusive-control-selector.md]] · [[soft-detent-interpolation.md]] · [[fade-then-snap.md]] · [[editor-preview-component.md]] · [[material-propertyblock-safe-update.md]] |
| Udon 沙箱适配 | [[hash-based-dispatch.md]] · [[iid-object-identity.md]] · [[slot-parameter-passing.md]] · [[code-generation-type-erasure.md]] · [[../../comparisons/build-time-vs-runtime.md]] |
| Pub/Sub 消息总线 | [[channel-routing.md]] · [[inherited-subscriber.md]] · [[hybrid-subscription-modes.md]] |
...
└── 动作（瞬时触发）→ Network Event（非模式）
   例: 音效、粒子、一次性动画 → [[entities/world/udon/networking/events]]
...
> **来源标注**:`⭐U` = UdonVoiceUtils 模式,以区别于 LuraSwitch2 模式(无后缀)。所有 UVU 模式从 （来源：UdonVoiceUtils 分析，本地整理） 提炼。
...
> **来源标注**:`⭐L` = ULocalization 模式(全称 Localization)以区别于其他来源。所有 UL 模式从 [ULocalization（GitHub）](https://github.com/ikuko/ULocalization) 提炼,聚焦于"如何用 Editor 端代码生成绕过 Udon VM 限制"。
...
| 关系 | 目录 |
|------|------|
| **规则约束** | [[concepts/rules/networking-rules]], [[concepts/rules/udonsharp-language-limits]] |
| **失败案例** | （来源：常见失败案例分析，本地整理） |
| **API 速查** | [[entities/api/networking]] |
| **应用层详解** | [[entities/world/udon/networking/index]] |
| **设计模式来源** | 3 个参考工程(多机位导演系统 / 视频播放器 / 音频同步系统) 已沉淀到 [[FACT]],具体项目信息见 源资料归档层（不随发布包分发） |
...
| 设计模式类别 | 主要模式 | 文档 |
|------|---------|------|
| **多机位导演系统(参考工程)** | 5 层同步 + NetworkCallable + 双缓冲 + 指数衰减 + Slerp + 所有权分层 + NoVariableSync UI | [[FACT]] §多机位导演系统 |
| **视频播放器(参考工程)** | Manual Sync + Owner Authority + 时间同步算法 | [[FACT]] §视频播放器 |
| **音频同步系统(参考工程)** | Shader-Centric + Master 时间锚点 + 位域压缩 + 漂移校正 | [[FACT]] §音频同步系统 |
| **QuickBrown LuraSwitch2(参考工程)** ⭐NEW | Master-Follower Syncer + Exclusive Selector + SoftDetent + Fade-Snap + Editor Preview + MaterialPropertyBlock | [LuraSwitch2（Lura Booth）](https://lura.booth.pm/) + 6 个新 pattern 文档 |
| **TLP UdonVoiceUtils(参考工程)** ⭐NEW | 10 个核心模式: Dual-Copy Sync + ExecutionOrder Chain + Strategy + Object Pool + Priority Arbitration + Master Ownership Defense + Gizmos + Trigger Fallback + Compile DEBUG + Auto Symbol | （来源：UdonVoiceUtils 分析，本地整理） + 10 个新 pattern 文档 |
| **HoshinoLabs ULocalization(参考工程)** ⭐NEW | 5 个 Udon 沙箱适配核心模式: Hash-Based Method Dispatch + IID Object Identity + Slot Parameter Passing + Code Generation Type Erasure + Build-Runtime Separation | [ULocalization（GitHub）](https://github.com/ikuko/ULocalization) + 5 个新 pattern 文档 |
| **HoshinoLabs Sardinal(参考工程)** ⭐NEW | 5 大 ULocalization 模式精简版 + 3 个 Sardinal 独有模式: Channel Routing + Inherited Subscriber + Hybrid Static+Dynamic Subscription | [Sardinal（GitHub）](https://github.com/ikuko/Sardinal) + 3 个新 pattern 文档 |

| **VRCLens(参考工程)** ⭐NEW 2026-07-04 | 5 个 P1 模式: HDR+White Balance + Single-Pass DoF + Avatar 相机限制修复 + 第三方 Tonemapper 集成 + 第三方 PostProcess 覆盖 | （来源：Open Source Projects，本地整理） §A10 + [[entities/avatar/camera-tools]] §3 |
