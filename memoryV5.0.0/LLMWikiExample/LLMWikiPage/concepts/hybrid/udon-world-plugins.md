...
#### 相比 [[entities/world/examples/mute-others]] 的升级
...
- **案例研究**: （来源：UdonVoiceUtils 分析，本地整理） (深度分析,10 章节,10 大难题,工程评价)
- **互补示例**: [[entities/world/examples/mute-others]] (官方基础版,适合学习)
- **设计模式**: [[concepts/patterns/index]] (10 个 ⭐U 模式: dual-copy-sync / execution-order-chain / strategy-pattern-udon / object-pool-udon / priority-arbitration / master-ownership-defense / gizmos-relationship / trigger-event-fallback / compile-time-debug-strip / auto-scripting-symbol)
- **API 索引**: （来源：UdonVoiceUtils 分析，本地整理） §8 (PlayerAudio* API 完整列表)
...
- **案例研究**: [VVMW（GitHub）](https://github.com/JLChnToZ/VVMW) (深度分析,5 大模块 + 13 个 Releases + 9 大集成 + 工程评价)
- **工具使用**: [[entities/world/vvmw]] (本节对应的完整工具文档)
- **时间同步算法模式**: [[FACT]] §视频播放器时间同步算法模式(参考工程)
- **互补官方示例**: [[entities/world/udon/video-players/index]] (SDK 基础视频播放器)
- **依赖生态**:
  - [[concepts/hybrid/audio-link]] AudioLink 集成
  - [[entities/world/vrc-light-volumes]] VRC Light Volumes 集成
  - [[entities/api/persistence]] PlayerData API
...
| 关系 | 目录 |
|------|------|
| **案例研究溯源** | （来源：Open Source Projects，本地整理）(所有推荐插件都必须先入案例研究) |
| **设计模式沉淀** | [[concepts/patterns/index]](插件实现的通用模式) |
| **官方示例互补** | [[entities/world/examples/index]](基础教学 vs 工业级) |
| **API 参考** | [[entities/api/index]](VRCPlayerApi 音频相关 API) |
| **失败案例参考** | （来源：常见失败案例分析，本地整理）(避免重蹈覆辙) |
| **Avatar 端对照** | [[concepts/hybrid/avatar-camera-plugins]](Avatar 端摄影工具,与本文档形成 World/Avatar 两端对照) |
...
| 2026-07-04 | 添加相关文档链接:`avatar-camera-plugins.md`(Avatar 端摄影工具索引,与本文档形成 World 端 vs Avatar 端对照) |

---

## 相关页面

[[alcom.md]] · [[audio-link.md]] · [[osc-protocol.md]] · [[vcc.md]] · [[index.md]] · [[avatar-camera-plugins.md]]
