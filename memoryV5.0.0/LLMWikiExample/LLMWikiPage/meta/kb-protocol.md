...
5. **网络来源准入**：网络搜索结果不得直接等同于知识库事实；官方资料、项目来源、社区经验需按 `network-search-protocol.md` 分级、验证与标注后再写入
...
| [[meta/index]] | 工作模式定义、维护协议、脚本说明 | 领域知识 |
...
| 协议 | 文件 | 用途 |
|------|------|------|
| 知识库写入与维护协议 | [[kb-protocol]] | 定义知识写入流程、Curator 维护流程与文件职责边界 |
| 网络搜索与知识入库准入协议 | [[network-search-protocol]] | 定义网络搜索来源分级、验证流程、禁止事项与入库标注规则 |
