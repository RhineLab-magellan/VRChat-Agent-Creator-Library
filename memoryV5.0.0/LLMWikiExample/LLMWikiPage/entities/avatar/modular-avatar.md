...
related:
  ...
  - ../../concepts/hybrid/avatar-camera-plugins.md
type: entity
created: 2026-07-04
sources: 本地知识库整理 + Modular Avatar GitHub releases (1.17.0, 2026-05-11; 1.18.0-alpha.0, 2026-05-31)
updated: 2026-08-13
---
# Modular Avatar (MA) 完整知识库
...
> **官方推荐**：使用 [ALCOM](https://vrc-get.anatawa12.com/en/alcom/)（VRChat Creator Companion 替代品）。VCC 仍受支持但不是推荐方式。
> ALCOM 详细文档见 [[concepts/hybrid/alcom]]。
> 官方原文："The recommended way to install Modular Avatar is using ALCOM."

```
1. ALCOM 添加仓库:
   - URL: https://vpm.nadena.dev/vpm.json
   - Repository Name: bd_
   - 点击 "Add VPM Repository"
2. 打开项目 "Manage Project" → 点击 Modular Avatar 旁的 +
3. 点击 APPLY → 装好了
```

> 更新版本：点击版本号旁的 **绿色箭头** → APPLY

### 2.1 预发布版本（Prerelease）

> ⚠️ **预发布版必须用 ALCOM**（VCC 依赖解析有 bug，官方明确要求）
> 官方原文："Due to bugs in VCC dependency resolution, please use ALCOM to install prerelease versions of Modular Avatar."
...
1. 打开 ALCOM
2. 进入 Settings 窗口
3. 勾选 "Show pre-release packages" 选项
4. 重新添加仓库（确保选择预发布版本）
5. 在 Manage Project 中选择预发布版本安装
```

**预发布版本仓库**：
- 主仓库：`https://vpm.nadena.dev/vpm.json`（在 bd_ 仓库中可切换 prerelease 标签）
- 文档：https://modular-avatar.nadena.dev/dev

> ⚠️ 预发布版本可能包含错误和兼容性问题；反馈请到 [GitHub Issues](https://github.com/bdunderscore/modular-avatar/issues)
...
> [[entities/avatar/modular-avatar-tutorials-detailed]] 教程 6](modular-avatar-tutorials-detailed.md#六教程-6-手动构建动画器为什么用)
...
- ⚠️ 对部分骨架(只有 Head 等单骨)会失效,需用 Bone Proxy 替代

> **完整决策树**:详见 `bone-binding-workflow.md` §决策树
> **对比表**:参见 §9.3.8
...
> **故障场景**:详见 `blend-shape-troubleshooting.md` §3.1 Shape Changer 冲突
...
| **Blend Shape 排障** | MA 组件冲突 / FX Layer 冲突 / WD 不一致 | 见 `blend-shape-troubleshooting.md` §4 步法 |
...
> **完整决策树(场景 → 推荐方法)**:详见 `bone-binding-workflow.md` §2 决策矩阵
> **手动绑定 Fallback**:当 MA 工具不可用时,详见 `bone-binding-workflow.md` §3.1
...
| 知识类别 | 文档位置 | 状态 |
|---------|---------|------|
| **官方教程** | [[entities/avatar/modular-avatar]]（本文）§6 | ✅ |
| **组件参考** | [[entities/avatar/modular-avatar]] §7 | ✅ |
| **反应式系统** | [[entities/avatar/modular-avatar]] §5 | ✅ |
| **高级话题** | [[entities/avatar/modular-avatar]] §9 | ✅ |
| **Samples 实战** | [[entities/avatar/modular-avatar]]（本文）§9.6 | |
| **教学法** | [[entities/avatar/teaching-methodology]] | ✅ |
| **教学法（MA 专项）** | [[entities/avatar/modular-avatar]] §4 | ✅（本文）|
| **创作者向完整版** | [[entities/avatar/modular-avatar]] §9.3 | |
...
**相关知识库**：
- [[entities/avatar/modular-avatar-tutorials-detailed]] — 教程精读详细版（27KB）
- [[entities/avatar/teaching-methodology]] — 教学法原则
- [[entities/avatar/animator-system]] — Animator 系统
- [[entities/avatar/ndmf-tools]] — NDMF 生态
- [[entities/avatar/optimization-guide]] — 性能优化
- [[entities/avatar/avatar-modding-guide]] — 改模指南
- [[concepts/hybrid/avatar-camera-plugins]] — Avatar 端摄影工具索引（含 Flex Fisheye Lens MA 实际应用案例）

## §15 MA 生态下的实际应用案例（Avatar 端插件）

> ⭐ **本节为 2026-07-04 新增，展示 MA 在第三方 Avatar 端插件中的实际应用**

### 15.1 Flex Fisheye Lens（Goat-Cannery）

**插件定位**：Avatar 端鱼眼相机摄影增强工具

**MA 应用方式**：
- 第三方插件通过 MA 实现"拖入 Prefab 即安装"
- Prefab 路径：`Assets/GoatCannery/FishEyeCam/FishEyeCam.prefab`
- 左撇子版本：`FishEyeCam_Left`
- 安装后自动在 Expressions 菜单添加 `[FishEyeLens]` 选项
- 自动在 Radial Menu 添加 `FishEyeCam` 菜单
...
**关键设计约束**：
- 🔴 插件仅支持 PC VR 模式，桌面和 Quest 不可用
- 🔴 必须使用最新版 MA，旧版可能出现导入错误
- 售价 ¥2,000（BOOTH）

**详情**：详见 [[concepts/hybrid/avatar-camera-plugins]] 完整文档

### 15.2 VirtualLens2（logi_9 / ろじらぼ）

> ⭐ **本节为 2026-07-04 新增，展示 MA 在第三方 Avatar 摄影工具中的标准应用**

**插件定位**：Avatar 端虚拟相机摄影增强工具（VRChat 摄影社区主流工具）

**MA 应用方式**：

| 步骤 | 操作 |
|------|------|
| 1 | 导入 VirtualLens2 unitypackage |
| 2 | 右键 Avatar 根 → `VirtualLens2` → `Non-Destructive Setup` |
| 3 | 配置设置（虚拟形象、目标手、设置模板等）|
| 4 | 点 `Apply` 按钮 |
| 5 | 重新上传 Avatar |

**MA 1.8.0+ 集成特性**：
- **最低 MA 版本**：1.8.0
- **非破坏性工作流**：通过 MA Menu Installer 集成到 Avatar 菜单
- v2.10.0 之前的破坏性设置可通过 `Migrate as a Non-destructive Modul` 按钮迁移
...
| 基础安装 | ✅ MA 是必需的 |
| 菜单自动生成 | ✅ MA 处理菜单挂载 |
| 参数同步 | ✅ MA Parameters 自动管理 |
| 升级兼容 | ✅ 迁移按钮支持旧版本数据迁移 |

**关键设计约束**：
- 🔴 VirtualLens2 主功能仅支持 **PCVR**（桌面 + VR）
- 🔴 Android 平台通过 **Remote Only Mode** 降级（仅显示头像）
- 表情参数需求：至少 8 bit 余量（1 个 Int 或 Float）
- 价格：¥1,500（BOOTH，Pay What You Want 模式）

**与其他工具的对比**：
- 详见 [[entities/avatar/camera-tools]] §2 / §4
- VRCLens（非破坏性反面）需 VRCFury Prefab 包装才能实现非破坏性

**详情**：详见 [[entities/avatar/camera-tools]] §2

**引用**：
- [L2] VirtualLens2 官方 Setup Guide: <https://vlens2.logilabo.dev/en/docs/getting-started/setup-guide/>
- [L2] VirtualLens2 中文文档: <https://docs.vrcd.org.cn/books/virtual-lens2>

---

## 16. 2026 版本增量

### 16.1 Modular Avatar 1.17.0（2026-05-11）

- 新增 `MA Floor Adjuster`。
- `MA Parameters` 支持 VRCRaycast components/parameters。
- `MA Bone Proxy` 增加 `Match scale`。
- Blendshape picker 支持多选；修复导入 World Project 时的编译错误。
- 针对 VRCSDK Expressions Menu 编辑器卡死加入规避；菜单 GUI 增加 watchdog 以便诊断渲染耗时。

来源：[FACT] [Modular Avatar 1.17.0](https://github.com/bdunderscore/modular-avatar/releases/tag/1.17.0)

### 16.2 Modular Avatar 1.18.0-alpha.0（2026-05-31，预发布）

- `MA Blendshape Sync` 支持使用曲线重映射 blendshape。
- Mesh Cutter 预览与构建性能提升。
- Mesh Deleter 预览逻辑调整以改善与其他插件的兼容性。
- **风险**：该版本是 prerelease，官方明确提示可能存在 bug 且 API 兼容性未保证；使用前应备份项目。

来源：[FACT] [Modular Avatar 1.18.0-alpha.0](https://github.com/bdunderscore/modular-avatar/releases/tag/1.18.0-alpha.0)

## 相关页面

[[modular-avatar-tutorials-detailed.md]] · [[ma-component-cards.md]] · [[ma2bt.md]] · [[avatar-modding-guide.md]] · [[lac-avatar-compressor.md]] · [[../../concepts/hybrid/avatar-camera-plugins.md]] · [[vrcraycast.md]] · [[ndmf-tools.md]]
