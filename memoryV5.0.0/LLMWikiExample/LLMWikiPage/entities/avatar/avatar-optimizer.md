...
version: 1.1
upstream_version: "v1.9.14 (2026-06-03); v1.9.13 verified 2026-05-13"
...
type: entity
created: 2026-06-21
sources: 本地知识库整理 + AvatarOptimizer GitHub releases (v1.9.13, 2026-05-13)
updated: 2026-08-13
---
# AAO: Avatar Optimizer 完整知识库
...
## 4.1 版本分层说明（v1.7.6 历史文档 vs v1.9+ 当前主线）

> ⚠️ 本知识库当前主线以 **v1.9.14 (2026-06-03)** 为准；若引用较早的汉化 / 社区整理文档（如 v1.7.6），必须明确标记为**历史版本说明**，不能回退成当前默认建议。

- **当前默认主线**：`Merge Material`、`Remove Mesh By Box`、`Remove Mesh By UV Tile`、`MeshRenderer 支持` 等 v1.9+ 能力。
- **历史版本说明**：`Merge ToonLit Material` 属于旧版组件路径，现阶段只应作为迁移 / 兼容说明保留。
- **吸收旧文的正确方式**：优先吸收 FAQ 证据、历史语境、已知冲突说明，而不是整段搬运旧组件手册。

---

### 4.2 AAO Trace And Optimize (T&O) ⭐核心
...
- 如果 T&O 改变了外观或行为 → 这应视为 **Bug**，应上报
- Debug Options 不受稳定性保证

#### FAQ / 已知风险补注（与当前主线仍相关）

- **OSC 开关 / 动画开关场景**：若 Avatar 某些 PhysBone、Contact Receiver、物件启用状态依赖动画或 OSC 驱动，`Remove Unused Objects` 等自动优化前应重点复核，避免把“静态看似未引用”的对象误判为可移除。
- **历史工具迁移**：`UnusedBonesByReferencesTool` 已属于旧路径；当前更推荐优先理解并使用 `Trace And Optimize` 中的 `Remove Unused Objects` 及相关保留选项。
- **吸收旧 FAQ 的原则**：可吸收“动画冲突 / 自动优化误删风险 / 旧工具已过时”等稳定结论，但不应把旧版本组件说明回退成当前主线。
...
| 问题 | 状态 | Issue | 解决方案 |
|------|------|-------|----------|
| **材质属性动画冲突** | 🔴 已知 bug | [#340](https://github.com/anatawa12/AvatarOptimizer/issues/340) | 合并网格时避免动画材质；冲突时显示警告需检查 console |
| **材质槽动画冲突** | ⚠️ 需手动配置 | — | 取消勾选 `Merge Materials` |
| **BlendShape 动画冲突** | 🔴 已知 bug | [#568](https://github.com/anatawa12/AvatarOptimizer/issues/568) | 合并时避免动画 BlendShape；冲突时显示警告需检查 console |

> 💡 **v1.9+ 新增能力**（v1.7.x 之前不存在）：
> - **Copy Enablement Animation**：复制合并目标 enablement 动画
> - **Auto Merge Material Slots**（`#1334`）：自动合并相同材质的槽
> - **Material Slots Mapping Fix**（`#1650`, 2026-02-14）：修复材质槽映射错误
...
| 版本 | 状态 | 详情 |
|------|------|------|
| **v1.7.6**（2024-06-17）| ✅ 主流组件 | 仅支持 `VRChat/Mobile/Toon Lit`，不支持第三方着色器 |
| **v1.9.x** | ⚠️ 标记废弃 | 官方文档标注 "This component is obsolete"，由 Merge Material 继任 |
| **下一 Major 版本** | ❌ 计划移除 | 官方 CHANGELOG 明确："Merge ToonLit Material will be removed in next major version" |

> 💡 **演进背景**：
> - PR #1516 引入 **Merge Material**（v1.9）
> - 支持更多 Shader：lilToon, ToonStandard, ToonLit 等
> - 通过 **Shader Information API** 扩展
> - 支持 **MeshRenderer**（v1.7.6 的 Merge ToonLit Material 仅支持 SkinnedMeshRenderer）

**升级建议**：
- 如果你**正在使用 v1.7.6 或更早**：继续使用 Merge ToonLit Material 是合理的
- 如果你**使用 v1.9+**：迁移到 Merge Material 获得更多 Shader 支持
...
**方案 1（推荐）**：创建 **Asset Description** 文件，配置 `Parameters Read By External Tools`

支持 4 种匹配模式：

| 模式 | 用途 | 示例 |
|------|------|------|
| **Exact** | 精确匹配参数名 | `_OSC_Toggle_Hat` |
| **Prefix** | 前缀匹配 | `_OSC_`（匹配所有 `_OSC_*`）|
| **Suffix** | 后缀匹配 | `_OSC`（匹配所有 `*_OSC`）|
| **Contains** | 子串匹配 | `OSC`（匹配所有包含 OSC 的参数）|

> 💡 **来源**：PR #1306（2024-11-02）实现了最小 OSC 支持，引用 Issue #1090。

**方案 2（兼容）**：将 OSC 使用的参数添加到 Expression Parameters 列表

> 旧方法，**仍可工作**但已不是首选。

**方案 3（兼容）**：在 Animator Controller 中添加 OSC 使用的参数

> 旧方法，T&O 不会删除被 Animator 引用的 PB/Contact，但**比方案 1 繁琐**。
...
**根因分析**:
- PhysBone/Contact Receiver 会生成**可被 OSC 工具读取**的参数
- 这些参数**无需在 Animator Controller 或 Expression Parameter 中定义**即可被 OSC 读取
- AAO 无法判断参数是"未使用"还是"专供 OSC 读取"
- 因此 AAO 默认假设未在 Animator 引用的参数是未使用，并删除对应的 PB/Contact 组件

**解决（按优先级）**：

| 方案 | 操作 | 难度 | 推荐度 |
|------|------|------|--------|
| **方案 A**（推荐）| 创建 Asset Description → `Parameters Read By External Tools` | 低 | ⭐⭐⭐ |
| **方案 B** | 在 Expression Parameters 列表添加 OSC 参数 | 中 | ⭐⭐ |
| **方案 C** | 在 Animator Controller 参数列表添加 OSC 参数 | 中 | ⭐ |

**Issue 追踪**: [#1090](https://github.com/anatawa12/AvatarOptimizer/issues/1090)（已部分解决，PR #1306）

> 💡 **商业化提示**：如果你的 OSC 开关是公开发布/销售的，可以联系作者把 Asset Description 集成到 AAO 中以获得更好的兼容性。
...
在上传前，模型游戏对象的右键菜单：`NDM Framework/Manual bake avatar`

> 插件会**克隆模型**，并将非破坏性工具应用于克隆体，保证原始模型不被修改。烘焙后的模型可直接上传。

#### 方法 2：Sayamame-beans/Upload-without-preCheck

- VPM 仓库：`https://sayabeans.github.io/vpm`
- 包名：`com.github.sayamame-beans.upload-without-precheck`
- 使用步骤：
  1. 打开 VRCSDK Control Panel
  2. 在 Hierarchy 中选择 Avatar
  3. 右键 → `[VRChat] Upload without pre-check`
- ⚠️ **兼容性警告**：VRCSDK 3.8.1-beta.1 因 `IVRCSdkAvatarBuilderApi.BuildAndUpload` 重载变更暂时破坏（需要等工具更新）
- 💡 **未来**：官方文档声明"如果 NDMF 或 VRCSDK 添加类似功能，此包将弃用"

#### 方法 3：VRCQuestTools（用于 Android/Quest）

- 官方文档：`https://kurotu.github.io/VRCQuestTools/`
- 包含 `VQT Avatar Builder`：允许在**不对 Android 版本进行预构建硬限制检查**的情况下上传
- 适合需要同时构建 PC + Quest 的用户

> 💡 **重要**：以上方法**不会跳过 *构建后* 的硬限制检查**（上传直前的检查）。如果模型真正超过限制，仍会上传失败。
...
### 8.6 合并 Skinned Mesh 材质/BlendShape 动画冲突

**问题**: `AAO Merge Skinned Mesh` 合并后，原目标网格的动画材质/动画 BlendShape 槽可能丢失或冲突。

**原因**:
- 材质属性动画冲突（已知 bug，[#340](https://github.com/anatawa12/AvatarOptimizer/issues/340)）
- 材质槽动画冲突（默认会合并动画材质插槽）
- BlendShape 动画冲突（已知 bug，[#568](https://github.com/anatawa12/AvatarOptimizer/issues/568)）

**解决**:
1. **材质属性动画冲突** — 避免将动画材质合并；冲突时 console 显示警告
2. **材质槽动画冲突** — 取消勾选 `Merge Materials` 中的 `Merge` 选项
3. **BlendShape 动画冲突** — 避免将动画 BlendShape 合并；冲突时 console 显示警告
...
| **v1.7.6** | Merge ToonLit Material 仍是主流材质合并组件，仅支持 `VRChat/Mobile/Toon Lit` |
...
| **v1.9.13** | 修复 Merge PhysBone 与动画/Constraints 父节点组合时的行为破坏；修复意外要求 VRCSDK 3.8.0+ 的兼容性回归；修复单侧眼骨导致 EyeLook 停止工作的情况（2026-05-13） |
| **v1.9.14** | **当前版本 (2026-06-03)** — 维护性更新 + 性能 Linting 持续迭代 |
| **v1.9.x** | Animator Optimizer (AnyState → Entry-Exit → BlendTree 转换)、Merge Material 继任 Merge ToonLit Material (PR #1516)、Max Texture Size 组件、Performance Linting |
...
[[ndmf-tools.md]] · [[accessories.md]] · [[animator-system.md]] · [[avatar-dynamic-bone-limits.md]] · [[avatar-fallback-system.md]]
