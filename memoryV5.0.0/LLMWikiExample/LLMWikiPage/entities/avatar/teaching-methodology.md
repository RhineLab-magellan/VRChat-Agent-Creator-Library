...
### 原则 11 — 对 Animator 新手先教“最小链路”

- 对只会用现成 Toggle、但不理解动画系统的学习者，优先讲清：
  - `Animator → Clip → Controller → Layer`
- 先让对方知道“参数驱动状态、状态播放片段、Layer 决定挂载位置”，再谈复杂框架（MA / VRCFury / BlendTree / WD）。
- 这样做的目标不是一次讲全，而是建立一个**可继续排错的骨架模型**。

### 可作为教学素材来源的文档类型（补充）
- **课程化 Unity 动画入门材料**：适合解释 Record / Preview、Dopesheet / Curves、关键帧与属性路径。
- **工具官方文档**：适合解释框架机制与安装要求。
- **现成 Prefab 拆解材料**：适合让学习者从可运行例子反推结构。

---

## §2.1 创作者分发责任链（VRCD 21 补充）⭐NEW 2026-07-04

> **来源**: [Avatar 文档（vrcd.org.cn）](https://docs.vrcd.org.cn/books/avatar) §-- 严正警告 --（VRCD 21 文档自带 2025 年自我修正）
> **本地化日期**: 2026-07-04
> **重要程度**: 🟡 教学场景补充,主要面向商品化 Avatar 创作者

### 0. 背景： "不要使用任何自动插件" 的演变

VRCD 21 文档原始立场:
> 在透彻了解 Avatar 前,**不要使用任何自动插件**（MA、VRCFury）

VRCD 21 文档 2025 年自我修正:
> 至 2025 年,我们有了 Modular Avatar 和 VRCFury 这两个自动化虚拟形象安装插件框架,创作者自己就必须先完全弄明白这个框架是怎么实现的,玩家也必须明白商品是怎么安装的。**"不要使用任何自动插件" 并非完全妥当**。

### 1. 责任链模型（原文提炼）

商品化 Avatar 的成功分发涉及**三方共担责任**:

| 方 | 责任 |
|----|------|
| **创作者** | 必须完全弄明白所用框架（MA/VRCFury）是怎么实现的;按最佳实践操作 |
| **框架作者** | 必须按照最佳实践实现;框架存在 bug 时承担修复责任 |
| **玩家** | 必须明白商品是怎么安装的;按文档说明操作 |

**关键结论**:
> "没有按照预期工作" 不是用户单方面的问题,而也有可能是创作者用户和框架作者没有按照最佳实践来操作导致的。

### 2. 对教学场景的应用

#### 2.1 教"商品化创作者" 时

- 不能简单说"别用 MA,先学 Avatar" → 立场已过时
- 应该说"**用 MA 前,先理解 MA 是怎么工作的**" → 立场升级
- 推荐阅读: MA 官方文档的"Merge Animator"、"Reactive Components" 等关键章节
- 推荐阅读: VRCFury 的"Prefab 转换"机制

#### 2.2 教"普通玩家" 时

- 不能假设玩家"会用" → 应提供图文安装步骤
- 应说明"如果 MA 没自动安装,如何手动 Setup Outfit"
- 应在商品说明中明确"所需依赖版本"（MA 1.17.1+ 等）

#### 2.3 教"教学者" 时

- 教学时主动说明三方责任,避免"甩锅给工具"
- 出现 Bug 时,先判断是"玩家操作问题"还是"框架问题"还是"创作者配置问题"
- 教"调试技巧" 比教"标准流程" 更重要

### 3. 教学话术建议

**过时话术** ❌:
> "MA 是给懒人用的,真正的高手都自己写 Animator"

**升级话术** ✅:
> "MA 是工具,不是替代品。**用 MA 前先理解 Animator**,这样 MA 出问题时你才能自己修复;做商品化 Avatar 时,玩家遇到问题你也才能帮上忙。"

### 4. 引用

- [L1] https://docs.vrcd.org.cn/books/avatar - VRCD 21「Avatar 与你」§严正警告（2025 年自我修正段落）
- [L2] https://modular-avatar.nadena.dev/docs/intro - MA 官方
- [L3] https://vrcfury.com/ - VRCFury 官方

---

## §2.2 输入输出法：功能设计三步法（VRCD 21 补充）⭐NEW 2026-07-04

> **来源**: [Avatar 文档（vrcd.org.cn）](https://docs.vrcd.org.cn/books/avatar) §第三部分「Avatar 功能的基本构成」
> **本地化日期**: 2026-07-04
> **重要程度**: 🟡 教学场景补充,适合教"想自己设计 Avatar 功能" 的新创作者

### 1. 输入输出法核心思想

> Avatar 的任何功能都可以用"输入输出法"描述。
> 功能 = **参数的变化** + **状态之间的过度条件** + **动画播放与否**

### 2. 三步法（VRCD 21 原文提炼）

**第一步: 识别输入（Parameter）**
- 这个功能需要哪些参数？（Bool / Int / Float）
- 玩家通过什么方式改变这些参数？（轮盘菜单 / Contact / OSC）

**第二步: 设计状态（State）**
- 这个功能有哪些状态？（默认 / 触发后 / 恢复中）
- 状态之间如何过渡？（Transition + Condition）
...
**第三步 - 动画**:
- State A → State B: 播放方块淡入动画
- State B → State A: 播放方块淡出动画

**Transition + Condition**:
- A → B: `cube_IsOn == true`
- B → A: `cube_IsOn == false`

### 4. 复杂武器功能示例（VRCD 21 原文）
...
开关功能:
  Parameter: 武器开关 (Bool)
  - 武器开关 = true  → 播放武器打开的动画
  - 武器开关 = false → 播放武器关闭的动画

开火功能:
  Parameter: 右手手势 + 武器开关
  - 右手手势 = 开枪手势 AND 武器开关 = true → 播放开火动画
  - 任一条件不满足 → 播放武器什么都不做
```
...
- 不直接教"如何做 Toggle",先教"如何构思功能"
- 用简单开关方块作为入门示例（3 状态 2 条件）
- 强调"先想清楚再动手做",避免"边做边改" 的混乱

#### 5.2 教"看了教程但不会变通" 的玩家

- 教程教的是具体实现,三步法教的是设计思路
- 教"识别输入 → 设计状态 → 设计动画" 的通用流程
- 玩家可以举一反三,设计新功能

#### 5.3 教"想做复杂功能" 的中级创作者

- 用武器示例展示"双条件组合" 的设计模式
- 教"参数依赖"（开火需要"武器打开" AND "右手手势" 双重条件）
- 引出更复杂的状态机设计（嵌套、AnyState 等）

### 6. 引用

- [L1] https://docs.vrcd.org.cn/books/avatar - VRCD 21「Avatar 与你」§输入输出法
...
> **学习来源**:官方 6 个教程的原文精读,详见 [[entities/avatar/modular-avatar-tutorials-detailed]]
...
**官方原文**（教程 1）:
> "Modular Avatar will automatically locate the armature object under the outfit, and attach a [Merge Armature](https://modular-avatar.nadena.dev/docs/reference/merge-armature) component to it."
...
| 文档 | 内容 |
|------|------|
| [[entities/avatar/teaching-methodology]] (本文) | 教学风格 + 具体技术提炼 + Samples 拆解教学法(原则 43-45) |
| [[entities/avatar/modular-avatar]] | MA 主文档(组件 + 教学决策树 + **§9.6 Samples 实战案例** ⭐NEW) |
| **[[entities/avatar/modular-avatar-tutorials-detailed]]** | 6 个教程的原文精读 + 玩家视角操作分解 |
| [[entities/avatar/avatar-modding-guide]] | 改模完整流程 |
...
> **学习来源**:本次系统学习官方 Component Reference 27 个组件 + 4 个横切页的原文,详见 [[entities/avatar/ma-component-cards]]
...
| 文档 | 内容 |
|------|------|
| [[entities/avatar/teaching-methodology]] (本文) | 教学风格 + MA 教程技术提炼 + 组件级语言转换 |
| [[entities/avatar/ma-component-cards]] ⭐NEW | 27 个组件的"教学卡"(When/How/Limitations) |
| [[entities/avatar/modular-avatar]] | MA 主文档(组件 + 教学决策树) |
| [[entities/avatar/modular-avatar-tutorials-detailed]] | 6 个教程的原文精读 |
| [[entities/avatar/avatar-modding-guide]] | 改模完整流程 |
...
> 整合官方 problems + 教学法原则
> **来源**: [[entities/avatar/modular-avatar]] §8.6
...
> 配套详细文档: [[entities/avatar/modular-avatar]] §9.6
...
| 文档 | 内容 |
|------|------|
| [[entities/avatar/teaching-methodology]] (本文) | 教学风格 + MA 教程技术提炼 + 组件级语言转换 + 问题诊断 + **边界场景与 FAQ 教学策略** |
| [[entities/avatar/ma-component-cards]] | 27 个组件的"教学卡"(When/How/Limitations) |
| [[entities/avatar/modular-avatar]] | MA 主文档(组件 + 教学决策树 + FAQ) |
| [[entities/avatar/modular-avatar-tutorials-detailed]] | 6 个教程的原文精读 |
| [[entities/avatar/avatar-modding-guide]] | 改模完整流程 |
...
[[animator-system.md]] · [[avatar-modding-guide.md]] · [[ma-component-cards.md]] · [[ma2bt.md]] · [[modular-avatar-tutorials-detailed.md]] · [[zh-translation-glossary.md]] · [[faceemo.md]] · [[lilycal-inventory.md]] · [[dressing-tools.md]]
