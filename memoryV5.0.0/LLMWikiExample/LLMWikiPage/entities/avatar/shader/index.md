...
type: entity
created: 2026-07-04
sources: "本地知识库整理 + VRChat 2026.1.2 Release Notes + Poiyomi 官方文档(2026-07-01)"
updated: 2026-07-04
---
# Avatar Shader 知识库索引
...
| Shader | 状态 | 重要变更 |
|--------|------|----------|
| **Toon Standard** | Avatar 默认 | 2026-02-19 起 `RenderType` = `Opaque`(原 Transparent) |
| **Toon Standard Outline** | Avatar 默认变体 | 同上 |

> **影响**:依赖 Transparent 渲染顺序的旧 Avatar 可能看起来不同。Opaque 通常性能更好、深度正确。
> **官方原话**(Developer Update 2026-02-19):"Our Toon Standard shaders, including the Outline variants, now set 'RenderType' = 'Opaque' in both client and SDK." — 来源: [VRChat Developer Update — 19 February 2026](https://ask.vrchat.com/t/developer-update-19-february-2026/47849)
...
| **Poiyomi** | VRChat 主流 Toon、5 个变体(Nano→Tera)、模块化系统、AudioLink、LTCGI | `[Poiyomi Shaders 官方文档](https://www.poiyomi.com/) ⭐NEW |
...
| Shader | 定位 | 说明 |
|--------|------|------|
| **Poiyomi Shaders** | Avatar Toon(主流) | 65 个文档已完整入库,涵盖 5 个 Shader 变体(Nano/Micro/Mega/Giga/Tera)、Pro 高级功能、模块化系统 → `[Poiyomi Shaders 官方文档](https://www.poiyomi.com/) ⭐2026-07-01 完成 |
...
| **Toon Standard (VRChat 官方)** | Avatar 默认卡通 | 2026-02-19 起 RenderType = Opaque(从 Transparent 变更),有 Outline 变体 |
...
├── [Poiyomi Shaders 官方文档](https://www.poiyomi.com/)  ← Poiyomi 65 个文档主索引
│
├── 🌍 World 专用
...
**详细文档**：[[entities/avatar/shader/liltoon/index]]（16个文件）
...
**详细文档**：[[entities/avatar/shader/scss]]
...
| **本地文档** | `[Poiyomi Shaders 官方文档](https://www.poiyomi.com/) 个 .md 文件) |
...
**详细文档**：`[Poiyomi Shaders 官方文档](https://www.poiyomi.com/) 个 .md 文件)
...
**详细文档**：[[entities/avatar/shader/orl/index]]（4个文件）
...
**详细文档**：[[entities/avatar/shader/filamented/index]]（4个文件）
...
**详细文档**：[[entities/avatar/shader/unlitwf/index]]
...
- [[entities/avatar/performance-rank]] — Avatar 性能排名
- [[entities/avatar/optimization-guide]] — Avatar 优化指南
- [[entities/avatar/ndmf-tools]] — NDMF 工具链
- [[entities/world/shader/index]] — World Shader 知识库
- [[entities/world/bakery/index]] — Bakery 光照烘焙

## 八、跨域兼容性注意事项

### 8.1 多相机架构与 UV 计算 Shader 的不兼容 ⚠️ 2026-07-04 新增

> 📌 **来源**: [[concepts/hybrid/avatar-camera-plugins]] (Flex Fisheye Lens Q&A Q4)

**问题描述**:
- 某些 Avatar 摄影工具(典型: Flex Fisheye Lens)使用**双相机架构**捕捉鱼眼效果
- 这类插件与**基于屏幕 UV 计算的 Shader 不兼容**
- 两个摄像头的 UV 拼接会导致基于 UV 的 Shader 计算错误

**受影响的 Shader 类型**:
- 后处理类(基于屏幕 UV)
- 全屏效果类
- 任何 `_ScreenParams` / `ComputeScreenPos` 计算
...
**世界搭建建议**:
- 创作者应在 World 中考虑这些摄影工具的兼容性
- 关键视觉特效避免过度依赖屏幕 UV 计算
- LOD 设计需考虑超广角相机会触发更远物体的 LOD 切换

### 8.2 相关文档

- [[concepts/hybrid/avatar-camera-plugins]] — Avatar 端摄影工具索引(包含完整的多相机兼容性说明)

---

## 九、SDK 内置 Toon Shader(2026-07-04 来自 22-7f6ff)

> [FACT] **来源**: https://creators.vrchat.com/platforms/android/quest-content-limitations + https://ask.vrchat.com/t/developer-update-april-24-2025/43071 + https://wiki.vrchat.com/wiki/Shaders
> **P 等级**: P1

### 9.1 Toon Standard(2025-04-24 新增,推荐)⭐

> [FACT] **VRChat 官方 Dev Update**: "we will have a new shader named Toon Standard... provide a powerful baseline that can run well on all hardware configurations that VRChat supports."

- **路径**: `VRChat/Mobile/Toon Standard`
- **平台**: PC + Quest 全部支持
- **特性**:
  - Detail Maps
  - Emission
  ...
  - Rim Light
  - 动画 Hue Shift
  - Custom Shadow Ramps(自定义阴影渐变)
  - Matcap(球形贴图)
  - Mask 纹理(多通道打包)
- **限制**: 不支持透明(transparency)、实时阴影
- **2026.1.2 变更**: `RenderType` = `Opaque`(原 Transparent)

### 9.2 Toon Lit(老, 2026 仍可用, 功能较弱)

> [FACT] **官方描述**: "Toon Lit is a very basic flat shader consisting of one feature: no shading is performed, whatsoever and is instead rendered flat."

- **路径**: `VRChat/Mobile/Toon Lit`
- **平台**: PC + Quest 全部支持
- **特性**: **基础平面着色**, 无阴影
- **限制**:
  - 无 Detail Maps
  - 无 Emission
  - 无 Rim Light
  - 无 Custom Shadow Ramps
  - **无光照计算**(完全平面)
- **推荐度**: 🟡 仅作兜底(无 Toon Standard 时的备选)

### 9.3 Toon Standard vs Toon Lit 对比

| 特性 | Toon Standard(新) | Toon Lit(老) |
|------|------------------|-------------|
| 平台 | PC + Quest | PC + Quest |
| 阴影 | ✅ Custom Ramps | ❌ 无 |
| Detail Maps | ✅ | ❌ |
| Emission | ✅ | ❌ |
| Rim Light | ✅ | ❌ |
...
详见: `[[entities/avatar/avatar-upload-getting-started]] §5.3`
