...
- **版本限制**: ✅ **仅 v1.3.0 及以下可用**
- **Quest 兼容**: ❌ 不可用
- **替代方案**: **VRChat PhysBones**（[[entities/avatar/index]] 中已记录）
...
| 现有知识 | 关联 |
|---|---|
| [[entities/api/not-exposed]] | 黑名单 API——其他第三方 Asset 不会暴露给 Udon |
| [[entities/world/whitelisted-world-components]] | 完整白名单组件清单 |
| [[entities/avatar/index]] | PhysBones 在 Avatar 域的详细文档 |
| [[entities/world/performance-guide]] | 后期处理对 GPU 的影响 |
| （来源：Example Central，本地整理） | Post Processing v2 实战案例 |
...
或者使用 **EasyQuestSwitch** 自动平台切换（[[entities/platform/easyquestswitch]]）。
...
- **相关知识**: [[entities/platform/easyquestswitch]] (PC/Quest 自动切换)
