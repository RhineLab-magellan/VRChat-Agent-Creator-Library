...
| # | 组件集合 | 数量 | 类别 |
|---|---|---|---|
| 1 | Unity Components | 77 | Unity 内置 |
...
### 1. Unity Components (77 个，官方 2026-11 核实)
...
- FlareLayer
- Halo
- LODGroup _（2026-08-19 补录：官方在线白名单在列 class-LODGroup.html，早期快照漏列）_
- LensFlare
- Light
- LightProbeGroup
...
- `Camera` 允许，但受 [[entities/world/vrc-camera-settings]] 限制
- `Light` 允许但需注意阴影/烘焙
...
| 现有知识 | 关联 |
|---|---|
| [[entities/api/not-exposed]] | **未暴露 API 黑名单**——非白名单 Script 直接失败 |
| [[entities/api/udon-type-exposure]] | Udon Type Exposure Tree 索引 |
| [[entities/api/exposed-types]] | 详细暴露类型清单 |
| [[concepts/rules/vrchat-api-exposure]] | API 暴露判断 |
| [[entities/avatar/vrc-constraints]] | VRC Constraints（与 Unity Constraints 对比） |
| [[entities/world/items]] | Layer / Mirror / Physics |
| [[entities/world/sdk-prefabs]] | SDK Prefab 使用的具体组件 |
| [[entities/world/supported-assets]] | 第三方 Scripted Asset（Final IK / Dynamic Bone / PPSv2 / AVPro） |
| [[entities/world/creator-economy]] | Store API 相关 |
| [[entities/world/udon/data-containers/index]] | Udon 数据容器 |

### Quest 平台限制

> "The Android version of VRChat has some exceptions to this list. Check [here](https://creators.vrchat.com/platforms/android/quest-content-limitations#components) for more info."

详细限制见 [[entities/platform/android-development]]。**关键差异**:
- TextMeshPro ✅ 保留
- PPSv2 ❌ 不可用
- Final IK ❌ 不可用
- Dynamic Bone ❌ 不可用
...
> "Maximum shape limits apply to these components. For more information, check the advice [here](https://creators.vrchat.com/worlds/components#world-shape-limits)."
...
2. **跨平台发布前**——使用 `EasyQuestSwitch` 自动处理 PC/Quest 差异（[[entities/platform/easyquestswitch]]）
...
- **相关知识**: [[entities/api/not-exposed]] (反向参考：黑名单)
- **相关知识**: [[entities/api/udon-type-exposure]] (类型暴露树)
- **相关知识**: [[entities/platform/android-development]] (Quest 限制)
- **相关知识**: [[entities/platform/easyquestswitch]] (平台切换工具)
