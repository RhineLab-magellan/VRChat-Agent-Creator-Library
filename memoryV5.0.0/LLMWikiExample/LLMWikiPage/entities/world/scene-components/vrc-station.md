...
> ✅ **注意**: 强制退座有**公开 API** —— `VRCStation.ExitStation(VRCPlayerApi player)`（配合 `UseStation(player)` 让玩家入座）。在 U# 中直接在 station 的 UdonBehaviour 上调用即可：
>
> ```csharp
> public override void Interact() {
>     station.ExitStation(remotePlayer);
> }
> ```
>
> 来源: [UdonSharp API — VRCStation](https://udonsharp.docs.vrchat.com/vrchat-api/#vrcstation)。如需按玩家强制退座，推荐 `ExitStation` 而非临时禁用组件/移动 Station。
