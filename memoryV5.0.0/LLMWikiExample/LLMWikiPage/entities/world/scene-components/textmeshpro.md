...
> ✅ **VRCTMPDropdownExtension**（SDK 3.6.1 起暴露）: 为 `TMP_Dropdown` 提供 `AddOptions` 扩展方法，在 U# 中可简洁地批量添加选项：
>
> ```csharp
> using VRC.SDK3.Components; // 必须引入
>
> dropdown.AddOptions(new string[] { "A", "B", "C" });   // string[]
> dropdown.AddOptions(sprites);                            // Sprite[]
> dropdown.AddOptions(optionDataArray);                    // TMP_Dropdown.OptionData[]
> ```
>
> 非扩展写法 `AddOptions(dropdown, options)` 同样可用。来源: 官方 [TMP Dropdown](https://creators.vrchat.com/worlds/components/textmeshpro/tmp_dropdown)。
...
VRChat 自带 [StringVariables 资产系统](../udon/data-containers/index.md),可通过 SDK 工具配置多语言字符串,运行时直接读取。
...
- TMP 子组件:
  - `TMP_Text` 基础类(TextMeshProUGUI / TextMeshPro)
  - `TMP_Dropdown`(下拉菜单)
  - `TMP_InputField`(输入框)
