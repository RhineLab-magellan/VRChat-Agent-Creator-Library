---
title: "Video Player Allowlist - 视频白名单"
category: world
subcategory: udon
...
changelog: "2026-07-04 新增 §Quest 端 YTDL 状态（2024 官方更新 + Discord 社区方案）"
confidence: Medium
tags:
  - world
  - udon
...
related:
  ...
  - ../data-containers/data-lists.md
  - ../data-containers/data-tokens.md
  - ../data-containers/index.md
type: entity
created: 2026-07-04
sources: 本地知识库整理
updated: 2026-07-04
...
## Quest 端 YTDL 状态（2026-07 更新）

> **来源**:
> - creators.vrchat.com/worlds/udon/video-players/（FACT-OFFICIAL）
> - protv.dev/urls/android-quest（FACT-AUTHORITATIVE）
> **状态**: ✅ FACT-OFFICIAL + FACT-AUTHORITATIVE

### 历史（2022-10 之前）

- **YouTube DL** 在 Quest 端**不可用**
- ProTV 官方解释: "Due to technical limitations beyond the VRChat dev's reach, YTDL is unable to be used natively on the Quest platform"
- 创作者必须用 `.mp4` 直链或第三方 Resolver

### 现状（2024-至今）

> **VRChat 官方原文（creators.vrchat.com）**:
...
- **现状**: Android 端（包括 Quest）**已有 URL 解析器**
- YouTube 等链接**部分可用**（稳定性问题仍存在）
- **ProTV 提示**:
  > "There are some stability issues known to exist in the Android build in relation to the YoutubeDL tool"

### 实操建议（按稳定性排序）

| 优先级 | 方案 | 稳定性 | 适用场景 |
|--------|------|--------|----------|
| ⭐⭐⭐ | 使用 `.mp4` / `.webm` 直链 | 最稳定 | 生产环境 |
| ⭐⭐ | 启用 Android 端 YTDL | 部分链接可工作 | 内部使用 |
| ⭐ | Discord 分发（社区方案） | 受 Discord 政策影响 | 个人/小规模 |
| ⭐ | 第三方 Resolver | 依赖工具维护 | 高级用户 |

### Discord 视频分发方案（社区方案）

> **来源**: VRCD 20-vowgan.md (COMMUNITY)
> **状态**: ⚠️ COMMUNITY（非官方）

**步骤**:
1. 上传 `.mp4` 视频到 Discord 频道
2. 复制文件直链（必须是 `.mp4` 结尾的直接 URL）
3. 在 VRC VideoPlayer 中使用该直链
...
- [VRChat Official] Video Players: <https://creators.vrchat.com/worlds/udon/video-players/>
- [ProTV] Android and Quest URLs: <https://protv.dev/urls/android-quest>
...
> **VRCUrl 是 Udon 访问外部 URL 的唯一方式**。详细 API 见 [[entities/world/udon/external-urls]]。
...
> Udon Type Exposure 状态: `VRCUrl` 暴露率 61.54% (8 个暴露成员),完整清单见 [[entities/api/exposed-types]]。
...
| 知识库 | 关系 |
|--------|------|
| [[entities/world/udon/video-players/index]] | 父页面(Video Players 总览) |
| [[entities/world/udon/external-urls]] | VRCUrl 类 + 白名单基础 |
| [[entities/world/udon/image-loading]] | 4KB 图像白名单(参考) |
| [[entities/world/udon/string-loading]] | VRCStringDownloader 白名单(参考) |
| [[entities/api/exposed-types]] | VRCUrl 完整暴露 API |
| [[entities/world/examples/udon-example-scene/udon-video-sync-player]] | UdonSyncPlayer 实现 |
| [[FACT]] → VVMW | 视频同步生产参考 |
| [[FACT]] → AudioLink | 音频零网络同步参考 |

---

## 参见

- **Video Players 总览**: [[entities/world/udon/video-players/index]]
- **VRCUrl & External URLs**: [[entities/world/udon/external-urls]]
- **Udon Type Exposure Tree**: [[entities/api/udon-type-exposure]]
...
[[../data-containers/byte-and-bit-operations.md]] · [[../data-containers/data-dictionaries.md]] · [[../data-containers/data-lists.md]] · [[../data-containers/data-tokens.md]] · [[../data-containers/index.md]]
