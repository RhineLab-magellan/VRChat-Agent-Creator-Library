...
related:
  ...
  - ../../../../concepts/patterns/late-joiner-state-restore.md
  - ../../../../concepts/patterns/bit-packed-flags.md
  - ../../../../concepts/patterns/advanced-sync-patterns.md
  - variables.md
type: entity
created: 2026-06-15
sources: "VRChat 官方 Creator Docs (Networking Index)"
updated: 2026-06-15
---
# Networking 概述
...
### 设计前先问自己（新增导览）
在决定 Manual / Continuous / Event / Ownership 之前，先回答：
1. 哪些状态必须让 late joiner 看见？
2. 哪些只是本地表现，不应消耗同步预算？
3. 谁拥有写权限 / 裁决权？
4. 对象复用或回收后，旧状态会不会泄漏？
5. 这个系统需要更多同步变量，还是先重构职责边界？

### 架构 / 模式补充入口
- `architecture.md` — 网络同步架构分型、设计问题清单
- `patterns.md` — 主动通知、PlayerId 传递、参数压缩等工程模式
...
| 文档 | 关系 |
|------|------|
| [[entities/api/networking]] | API 速查(Networking 类、UdonBehaviour 同步成员) |
| [[concepts/patterns/manual-sync-state]] | Manual Sync 模式实现范式 |
| [[concepts/patterns/owner-authoritative-interaction]] | Owner Authority 模式 |
| [[concepts/patterns/late-joiner-state-restore]] | Late Joiner 状态恢复 |
| [[concepts/patterns/bit-packed-flags]] | 位域压缩同步(节省带宽) |
| [[concepts/patterns/advanced-sync-patterns]] | 高级同步模式 |
| [[entities/world/udon/networking/variables]] | UdonSynced 字段类型详解 |
| [[entities/world/udon/networking/tools/index]] | **开发/调试工具**(Network ID Utility + Network Stats API) |
...
| [ClientSim（VRChat 官方）](https://creators.vrchat.com/worlds/clientsim/) + [[entities/world/clientsim/index]] | ClientSim 项目仓库元数据 + 4 个调试窗口 | `clientsim.md` |
