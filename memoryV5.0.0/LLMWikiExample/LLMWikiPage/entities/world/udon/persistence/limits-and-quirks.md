...
related:
  ...
  - player-object.md
  - serialization.md
source: VRChat Creator Docs(https://creators.vrchat.com/worlds/udon/persistence/) + VRChat 2026.1.2p2 Release Notes
...
type: entity
created: 2026-06-30
sources: VRChat Creator Docs(https://creators.vrchat.com/worlds/udon/persistence/) + VRChat 2026.1.2p2 Release Notes
updated: 2026-06-30
---
# 100KB 限制与常见陷阱
...
| **ClientSim** | 项目内 JSON 文件 | 见 [ClientSim（VRChat 官方）](https://creators.vrchat.com/worlds/clientsim/) |
...
> **FACT** (2026.1.2p2):"Fixed an issue where data for the last player to leave an instance could be lost if the instance shut down quickly."

### 问题场景

| 场景 | 表现 | 修复状态 |
|------|------|----------|
| **最后玩家快速离开** | 该玩家的 PlayerData / PlayerObject **可能丢失** | ✅ 2026.1.2p2 修复 |
| **实例快速关闭** | 部分 PlayerData 写入未完成即丢失 | ✅ 2026.1.2p2 修复 |

### 创作者影响

| 影响项 | 说明 |
|--------|------|
| **数据安全** | 减少"最后玩家"边界情况下的数据丢失 |
| **无需代码改动** | 纯引擎层修复 |
| **最佳实践不变** | 仍建议 `OnPlayerLeft` 主动 Save |
| **旧版客户端** | 2026.1.2p1 之前可能仍触发此问题 |

### 历史问题(2026.1.2p2 之前)

> **【未确认】** 以下边缘情况在 2026.1.2p2 之前可能存在:
> - 实例在 30 秒内被关闭(快速循环测试)时数据丢失
> - Late Joiner + 快速退出组合场景

### 关联建议

```csharp
// 即使有 2026.1.2p2 修复,仍建议主动保存
public override void OnPlayerLeft(VRCPlayerApi player)
{
    if (player == Networking.LocalPlayer)
    {
        // 本地玩家离开前主动保存
        SavePlayerData();  // 详见 serialization.md
    }
}
```
...
- [[entities/api/persistence]] - API 速查
- [[concepts/patterns/late-joiner-state-restore]] - Late Joiner 状态恢复
- [[concepts/patterns/manual-sync-state]] - Manual Sync 模式
- [[concepts/patterns/bit-packed-flags]] - 位域压缩同步
- [[entities/world/udon/networking/performance]] - 网络性能优化
- [[entities/world/udon/event-execution-order]] - 事件时序
- [ClientSim（VRChat 官方）](https://creators.vrchat.com/worlds/clientsim/) - ClientSim 调试

---

## 相关页面

[[../../../api/persistence.md]] · [[../index.md]] · [[player-data.md]] · [[player-object.md]] · [[serialization.md]]
