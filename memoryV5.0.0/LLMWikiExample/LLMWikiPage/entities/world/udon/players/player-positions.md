---
title: "Player Positions — 玩家位置/旋转/追踪/速度/传送"
category: world
subcategory: udon
knowledge_level: applied
status: active
source: "https://creators.vrchat.com/worlds/udon/players/player-positions/ (Last updated: 2024-12-13) + VRChat 2026.2.3 Release Notes"
...
type: entity
created: 2026-06-30
sources: "https://creators.vrchat.com/worlds/udon/players/player-positions/ (Last updated: 2024-12-13) + VRChat 2026.2.3 Release Notes + https://ask.vrchat.com/t/developer-update-13-august-2026/48800"
updated: 2026-08-19
---
# Player Positions — 玩家位置/旋转/追踪/速度/传送

> Subtype: API 详解
> 底层引用: [[entities/api/player-api]]
> 抓取日期: 2026-06-15
...
### 原生骨骼挂载（AttachTransformToBone / DetachTransformFromBone · PLANNED）

> ⚠️ **[PLANNED] 未正式发布** —— 官方 2026-08-13 开发者更新宣布，签名/行为/可用 SDK 版本正式发布前可能变动。官方明确标注 "subject to change until release"。

**动机**：传统需要在 Update 循环中每帧手动移动/旋转 Transform 跟随骨骼，Udon 中开销较大：
...
**新方法（一次性调用，VRChat 自动处理骨骼追踪）**：

```csharp
// 挂载：将 transform 吸附到本地玩家 Avatar 的指定骨骼
...
| 方法 | 说明 |
|------|------|
| `AttachTransformToBone(transform, HumanBodyBones)` | 一次性挂载到骨骼；之后每帧追踪由 VRChat 原生处理，不再需要 Udon update loop |
| `DetachTransformFromBone(transform)` | 解除挂载 |

> **FACT**（官方 2026-08-13 公告）：只需调用一次，挂载后由 VRChat 自动处理骨骼追踪。
>
> **[COMMUNITY]** 官方回复（Dexvoid，同一帖子）：本能力定位为"更便捷且更高效的逐帧骨骼追踪（Update 方式）的**一对一等价替换**"；culled / hidden avatar / imposter / fallback 等情形的表现与上述 Update 循环方式一致，**远程玩家骨骼旋转不一致的历史问题并未由本 API 消除**（有创作者在该帖反馈）。
...
> **不要在网络更新期间传送玩家**(如 [OnDeserialization](../networking/network-components.md#ondeserialization))。否则玩家的 Avatar 可能在传送过程中意外与环境碰撞。
>
...
### 🔄 VRChat 修复历史 (2026.2.3+)

> **FACT** (2026.2.3 Build 1864,2026-06-22):"Fixed an issue with Udon's `Teleport` functions not working if the rotation quaternion was not unit-length. This is the case in Udon Graph by default, which is where this fix will mostly apply."

#### 问题场景

| 场景 | 表现 | 修复状态 |
|------|------|----------|
| **Udon Graph 默认旋转** | Udon Graph 创建的 `Teleport` 节点使用**非单位长度四元数**,导致 Teleport 不工作 | ✅ 2026.2.3 修复 |
| **Udon# 代码** | 显式 `player.TeleportTo(pos, rot)` 使用单位四元数时正常,非单位时异常 | ✅ 2026.2.3 修复 |
| **Late Joiner 场景** | 新加入玩家在 OnNetworkReady 触发的传送现在可靠 | ✅ 2026.2.3 修复 |

#### 创作者影响

| 影响项 | 说明 |
|--------|------|
| **Udon Graph 用户** | 默认 `Teleport` 节点现在可靠工作(无需手动归一化) |
| **Udon# 用户** | 显式 `player.TeleportTo` 现在对单位/非单位四元数都正确处理 |
| **延迟一帧仍是最佳实践** | 修复后仍建议 `SendCustomEventDelayedFrames(..., 1)` 避免碰撞异常 |
...
public void DelayedTeleport()
{
    // 2026.2.3+ 即使不归一化也能工作,但归一化是良好实践
    Quaternion normalizedRot = targetRot.normalized;
    Networking.LocalPlayer.TeleportTo(targetPos, normalizedRot);
}
```
...
> 2026.2.3 同时发布:
> - **Outgoing Queue Drop Policy**(详见 [[entities/api/networking]] §Outgoing Queue Drop Policy)
> - **VRCObjectSync 可靠性增强**(详见 [[entities/world/scene-components/vrc-objectsync]] §2026.2.3 可靠性增强)
> - **Udon Teleport 四元数归一化**(本节)
...
| 知识库 | 关系 |
|---|---|
| [[entities/api/player-api]] | 底层 API 简略版 |
| [[entities/world/udon/players/player-forces]] | 移动/速度/力相关 |
| [[entities/world/udon/players/player-avatar-scaling]] | `GetPlayerEyeHeightAsMeters` |
| [[entities/world/udon/networking/index]] | 网络更新事件、传送同步 |
| [[entities/world/udon/networking/network-components]] | `OnDeserialization` 注意事项 |
| [[entities/api/vrc-billboard]] | 同期预告：原生公告板 API 类（VRCBillboard） |

---

## 相关页面

[[player-forces.md]] · [[player-audio.md]] · [[player-avatar-scaling.md]] · [[getting-players.md]] · [[player-collisions.md]] · [[../../../api/vrc-billboard.md|VRCBillboard（同期预告）]]
