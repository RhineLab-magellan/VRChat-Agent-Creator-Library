...
related:
  ...
  - udon/video-players/www-whitelist.md
  - ../../concepts/hybrid/audio-link.md
  - vrc-light-volumes.md
  - FACT.md
  - ../../concepts/patterns/manual-sync-state.md
  - udon/external-urls.md
  - ../api/persistence.md
...
### 通用模式（8 个）→ [[FACT]] + [[concepts/patterns/index]]
...
| 播放后端抽象 | ``策略模式（模式库待整理，见 [[code-generation-type-erasure]]）`` | `Handler` 接口 |
| 字符串拼接避冲突 | [[byte-and-bit-operations]] | `\u2029` Paragraph Separator |

### 案例研究档案 → [VVMW（GitHub）](https://github.com/JLChnToZ/VVMW)
...
**作为知识来源时**：
> 来源：[VVMW（GitHub）](https://github.com/JLChnToZ/VVMW)（案例研究 + 8 个 Pattern 来源）
> 工具使用：[[entities/world/vvmw]]（本文件）
...
| 文档 | 关系 |
|------|------|
| [VVMW（GitHub）](https://github.com/JLChnToZ/VVMW) | **本项目的案例研究型源文档**（A2） |
| [[entities/world/udon/video-players/index]] | 视频播放器总览（SDK 基础） |
| [[entities/world/udon/video-players/www-whitelist]] | 视频 URL 白名单 |
| [[concepts/hybrid/audio-link]] | AudioLink 集成（VVMW 集成之一） |
| [[entities/world/vrc-light-volumes]] | VRC Light Volumes（VVMW 集成之一） |
| [[FACT]] §视频播放器 | **时间同步算法模式（参考工程）** |
| [[concepts/patterns/manual-sync-state]] | Manual Sync 基础 |
| [[entities/world/udon/external-urls]] | VRCUrl 基础 |
| [[entities/api/persistence]] | PlayerData API（持久化用） |
| [[entities/world/performance-guide]] | World 性能优化（多实例屏幕场景） |
| [[entities/world/udon/video-players/index]] | 视频播放器底层知识 |
| [[concepts/hybrid/udon-world-plugins]] | 推荐 Udon 世界插件索引 |
