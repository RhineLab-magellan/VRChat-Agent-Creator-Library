...
**适用 Physics 方法**: 详见 [[entities/world/layers]] → "Physics and Layers" 章节。
...
| 现有知识 | 关联 |
|---|---|
| [[entities/world/layers]] | Layer 3 (`Item`) 定义、Physics 方法清单、LayerMask 用法 |
| [[entities/api/udon-type-exposure]] | `Utilities.IsValid` API（Udon 暴露的 Unity Object 验证方法） |
| [[concepts/rules/networking-rules]] | Owner Authority、Manual Sync、连续同步模式 |
| [[entities/api/pickups]] | Pickup 系统（与 Item 不同的概念，Pickup 是 World 自带的对象） |
